...
class Responder:
    ...
    def __init__(
        ...
        emotional_journal: EmotionalJournal = None,
    ) -> None:
        """
        ...
        self.user_profile = Profile(
            user_profile=user_profile, gpt_model=self.settings.profiler_gpt_model
        )
    ...
    async def handle_user_message(
        self, user_message: str, use_tools: bool, extract_inputs: bool
    ) -> (str | None, dict | None, str | None, str | None):
        """
        ...
        flagged, flagged_categories = self.moderation.moderate(
        ...
        if self.message_count % self.settings.messages_till_profile_update == 0:
            profile_update_task = asyncio.create_task(
                self.user_profile.update_user_profile(
                    previous_messages=self.chat_history[
                        : self.settings.messages_for_profile_update
                    ],
                    user_message=user_message,
                )
            )
        ...
        if self.message_count % self.settings.messages_till_journal_update == 0:
            journal_update_task = asyncio.create_task(
                self.emotioal_journal.update_journal(
                    chat_history=self.chat_history[
                        -self.settings.messages_for_journal_update :
                    ]
                )
            )
        ...
        system_message += "\n" + personality_addition(
            self.settings.responder_personality
        )

        prompt += responder_prompt(
            chat_history="\n".join(
                self.chat_history[-CHAT_HISTORY_MESSAGES_FOR_RESPONDER:]
            ),
            user_message=user_message,
        )
        print(
            f"- _Responder_ Sysytem prompt:\n{system_message}\n- Prompt:\n{prompt}\n{'_'*100}"
        )
    ...
    async def handle_user_inputs(
        self, tool: str, inputs: dict[str, str]
    ) -> (str, dict[str, str]):
        """
